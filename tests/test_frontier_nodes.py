...
class FrontierGraphTests(unittest.TestCase):
    ...
    def test_area_import_connects_heat_context_and_authored_growth(self):
        ...
        self.assertTrue(material["carbon_frontier_vertex_view"])
    ...
    def test_fx_fresnel_preserves_interpolated_view_and_independent_alpha(self):
        tree = self.frontier.build_group(self.member("fxv5"))
    ...
    def test_fx_vertex_view_tracks_camera_replacement_and_parent_shear(self):
        ...
        cameras = []
        try:
            modifier = self.frontier.attach_fx_vertex_view(obj, bpy.context.scene)
            self.assertEqual(self.frontier.attach_fx_vertex_view(obj, bpy.context.scene), modifier)
